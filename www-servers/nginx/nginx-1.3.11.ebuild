...
NGX_PAGESPEED_MODULE_PV="master"
NGX_PAGESPEED_MODULE_P="ngx_pagespeed"

# nginx-auth-ldap (https://github.com/kvspb/nginx-auth-ldap)
NGINX_AUTH_LDAP_MODULE_PV="master"
NGINX_AUTH_LDAP_MODULE_P="nginx-auth-ldap"
...
SRC_URI="http://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_ngx_pagespeed? ( https://github.com/pagespeed/ngx_pagespeed/archive/master.zip -> ${NGX_PAGESPEED_MODULE_P}-${NGX_PAGESPEED_MODULE_PV}.zip )
	nginx_modules_nginx-auth-ldap? ( https://github.com/kvspb/nginx-auth-ldap/archive/master.zip -> ${NGINX_AUTH_LDAP_MODULE_P}-${NGINX_AUTH_LDAP_MODULE_PV}.zip )"
...
http_upload http_ey_balancer http_slowfs_cache ngx_pagespeed nginx-auth-ldap"
...
CDEPEND="
	...
	nginx_modules_nginx-auth-ldap? ( net-nds/openldap )"
...
src_prepare() {
	...
	if use nginx_modules_nginx-auth-ldap; then
		EPATCH_OPTS="-d ${WORKDIR}/${NGINX_AUTH_LDAP_MODULE_P}-${NGINX_AUTH_LDAP_MODULE_PV}" epatch "${FILESDIR}"/ngx_http_auth_ldap_module_tls_cert_warn.patch
	fi

	epatch "${FILESDIR}"/satisfy_any_unauth_forbidden.patch
}
...
src_configure() {
	...
	if use nginx_modules_ngx_pagespeed; then
	    myconf="${myconf} --add-module=${WORKDIR}/${NGX_PAGESPEED_MODULE_P}-${NGX_PAGESPEED_MODULE_PV}"
	fi

	if use nginx_modules_nginx-auth-ldap; then
	    myconf="${myconf} --add-module=${WORKDIR}/${NGINX_AUTH_LDAP_MODULE_P}-${NGINX_AUTH_LDAP_MODULE_PV}"
	fi
